...
*/}
      ...
      <div>
        {Object.entries(fetchedMaterial || {}).map((k) => {
          if (getValues('selectedMat') !== '' && k[0] === 'name' ) {
            return  (
            <div className=" flex flex-row gap-2 flex items-center ">
              <div className="basis-1/2 md:basis-1/5">
              <label className="block text-sm font-medium text-gray-700 ml-8 ">{k[0]}:{' '}</label>
              </div>
              <div className="basis-1/2 md:basis-1/3 -mx-8">
              <input {...register('name')}
          type="text"
        ...
        />
         ...
         {console.log('k1',k[1])}
          ...
          if (getValues('selectedMat') !== '' && k[0] === 'type' ) {
            const opts=['cathode', 'anode', 'electrolyte']
            return  (
            <div className=" flex flex-row gap-2 flex items-center ">
              <div className="basis-1/2 md:basis-1/5">
              <label className="block text-sm font-medium text-gray-700 ml-8 ">{k[0]}:{' '}</label>
...
{console.log('name',getValues('name'),'type',getValues('type'),'selmat',getValues('selectedMat'))}
