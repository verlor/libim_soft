...
export default function ParameterFormular() {
  ...
  const lastCreated = useSelector((state) => state.materials.isCreated)
  ...
  useEffect(() => {
    if (
      initFormState.fast_charge_rate_id !== '-1' &&
      initFormState.slow_charge_rate_id !== '-1'
    ) {
      setValue('slow_charge_rate_id', initFormState.slow_charge_rate_id)
      setValue('fast_charge_rate_id', initFormState.fast_charge_rate_id)
    }
  })

  useEffect(() => {
    if (lastCreated.matCreated == true) {
      setTimeout(() => {
        dispatch(
          setIsCreated({
            matCreated: false,
  ...
  }, [lastCreated.matCreated])

  return (
    <>
      {lastCreated.matCreated && (
        <NewMaterialAlert
          mName={lastCreated.matName}
          mType={lastCreated.matType}
        />
      )}
      ...
      <form
        onSubmit={handleSubmit(async (form_data) => {
          ...
          dispatch(
            updateFormState({
              ...form_data,
              slow_charge_rate_id: parseFloat(form_data.slow_charge_rate_id),
          ...
          const foData = {
            ...
            cathode_theor_capacity:
              respCathode.properties.cathode_theor_capacity,
            sr_cathode_capacity:
            ...
            <select
              {...register('fast_charge_rate_id', {
                ...
                validate: {
                  gtr: (v) =>
                    parseFloat(v) >
                    parseFloat(getValues('slow_charge_rate_id')),
                },
              })}
